...
<!-- fable-harness v1.1 (2026-07-04) — hook-based guardrails for the fable frameworks.
     v1.1: optional strict verify (FABLE_HARNESS_VERIFY_CMD) + kill switch note.
...
- **Strict verify (where enabled).** If `FABLE_HARNESS_VERIFY_CMD` is configured, the Stop hook also runs the project's real check command after edits and blocks completion while it fails. Fix the failures — never work around the check.
